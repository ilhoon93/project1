...
type Category = '알림장' | '웨딩스냅' | '공통';

interface QA {
  q: string;
  a: React.ReactNode;
  /** 어떤 탭에서 보일지. '공통'은 두 탭 모두 노출. */
  category: Category;
}
...
const FAQS: QA[] = [
  ...
  {
    category: '웨딩스냅',
    q: '결과가 마음에 안 들면 다시 만들 수 있나요?',
    a: (
      <>
        재생성은 <strong>무료 재생성 크레딧(횟수)</strong> 내에서 할 수 있어요. 무료 재생성은
        크레딧/패키지 구매 시 함께 지급되며, 모두 소진하면 추가 재생성에는 스냅 크레딧이
        사용됩니다. 한편 <strong>시스템 오류</strong>로 생성에 실패한 경우에는 사용된 스냅 크레딧이
        자동으로 환원됩니다.
      </>
    ),
  },
...
const TABS: Category[] = ['알림장', '웨딩스냅'];

export function FaqClient() {
  const [tab, setTab] = useState<Category>('알림장');
  const items = FAQS.filter((f) => f.category === tab || f.category === '공통');

  return (
    <>
      <div className="mt-6 inline-flex rounded-full border border-[var(--wd-line)] bg-white p-1">
        {TABS.map((t) => (
          <button
            key={t}
            type="button"
            ...
            className={`rounded-full px-4 py-1.5 text-[13px] font-medium transition-colors ${
              tab === t
                ? 'bg-[var(--wd-ink)] text-[var(--wd-cream)]'
